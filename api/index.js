    ...
    try{
      ...
      const r=await fetch('https://auth.opensky-network.org/realms/opensky-network/protocol/openid-connect/token',
        {method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},
         body:new URLSearchParams({grant_type:'client_credentials',client_id:cid,client_secret:cs}),signal:ctrl.signal});
      if(!r.ok)return null;
...
async function handleOpenSky(req,res) {
  ...
  try{
    const ctrl=new AbortController(); setTimeout(()=>ctrl.abort(),25_000);
    const upstream=await fetch('https://opensky-network.org/api/states/all?extended=1',{headers,signal:ctrl.signal});
    const body=await upstream.text();
    if(upstream.status===429){_osCooldown=now+120_000;if(_osCache)return sr(res,200,_osCache,'application/json');return sj(res,429,{error:'OpenSky rate limited'});}
    if(upstream.ok){_osCache=body;_osCacheAt=now;_osCooldown=0;}
    return sr(res,upstream.status,body,'application/json');
  }catch(err){
...
async function handleOpenSkyTrack(req,res) {
  ...
  try{
    ...
    const r=await fetch(u.toString(),{headers,signal:ctrl.signal});
...
async function handleAdsbdb(req,res) {
  ...
  try{
    ...
    const r=await fetch(upUrl,{signal:ctrl.signal});
    if(r.status===404){store.set(key,{at:now,data:null});return sj(res,200,{data:null});}
    if(!r.ok)throw new Error(`HTTP ${r.status}`);
    const json_=await r.json(); let data=null;
    if(kind==='route'){const fr=json_?.response?.flightroute;if(fr?.origin&&fr?.destination){const ap=a=>({code:a.iata_code||a.icao_code||'',name:a.municipality||a.name||'',lat:a.latitude??null,lon:a.longitude??null});data={airline:fr.airline?.name||null,origin:ap(fr.origin),destination:ap(fr.destination)};}}
    else{const a=json_?.response?.aircraft;if(a)data={typeCode:a.icao_type||null,typeName:a.manufacturer&&a.type?`${a.manufacturer} ${a.type}`:(a.type||null),registration:a.registration||null};}
...
async function handleAdsbLolMil(req,res) {
  ...
  try{
    const ctrl=new AbortController(); setTimeout(()=>ctrl.abort(),10_000);
    const r=await fetch('https://api.adsb.lol/v2/mil',{headers:{'User-Agent':'gods-eye-view/1.0'},signal:ctrl.signal});
...
async function handleAdsbLolTrace(req,res) {
  ...
  try{
    const ctrl=new AbortController(); setTimeout(()=>ctrl.abort(),10_000);
    const r=await fetch(`https://api.adsb.lol/v2/icao/${hex}`,{headers:{'User-Agent':'gods-eye-view/1.0'},signal:ctrl.signal});
    return sr(res,r.status,await r.text(),'application/json');
  }catch(err){return sj(res,502,{error:err.message});}
}

// ─────────────────────────────────────────────────────────────────────────────
// CELESTRAK — uses gp.php?GROUP= (NOT /pub/TLE/ — that returns 403 for bulk)
// ─────────────────────────────────────────────────────────────────────────────
...
async function handleCelestrak(req,res) {
  ...
  try{
    ...
    const ctrl=new AbortController(); setTimeout(()=>ctrl.abort(),20_000);
    const r=await fetch(url.toString(),{headers:{'User-Agent':'gods-eye-view-celestrak-proxy/1.0 (+https://github.com/bilawalsidhu/gods-eye-view)'},signal:ctrl.signal});
...
async function handleLaunches(req,res) {
  ...
  try{
    ...
    const ctrl=new AbortController(); setTimeout(()=>ctrl.abort(),20_000);
    const r=await fetch(u.toString(),{headers,signal:ctrl.signal}); const body=await r.text();
...
async function handleHudSummary(req,res) {
  ...
  if(!apiKey)return sj(res,503,{error:'OPENAI_API_KEY is not set'});
  let context={}; try{context=JSON.parse((await readBody(req))||'{}');}catch{}
  try{
    const ctrl=new AbortController(); setTimeout(()=>ctrl.abort(),15_000);
    const r=await fetch('https://api.openai.com/v1/responses',{
      method:'POST',headers:{Authorization:`Bearer ${apiKey}`,'Content-Type':'application/json'},
      body:JSON.stringify({model:process.env.OPENAI_HUD_SUMMARY_MODEL||'gpt-5-nano',
        instructions:"Write one concise intelligence-HUD summary for God's Eye View. Output exactly five words with no title, punctuation, or markdown.",
        input:JSON.stringify(context),reasoning:{effort:'minimal'},max_output_tokens:100}),signal:ctrl.signal});
...
async function handleRealtimeToken(req,res) {
  ...
  try{
    ...
    const r=await fetch('https://api.openai.com/v1/realtime/client_secrets',{
      ...
      body:JSON.stringify({session:{type:'realtime',model,reasoning:{effort},
        truncation:{type:'retention_ratio',retention_ratio:contextRetention,token_limits:{post_instructions:contextTokens}},
        audio:{input:{noise_reduction:{type:'near_field'},turn_detection:{type:'semantic_vad',eagerness:'low',create_response:true,interrupt_response:false}},output:{voice}},
        instructions:"You are GEV Voice Control for God's Eye View. Respond concisely.",tool_choice:'auto'}})});
    const body=await r.text();
...
async function handleNearbyPlaces(req,res) {
  ...
  try{
    const ctrl=new AbortController(); setTimeout(()=>ctrl.abort(),10_000);
    const r=await fetch('https://places.googleapis.com/v1/places:searchNearby',{method:'POST',signal:ctrl.signal,
...
async function handleTextSearch(req,res) {
  ...
  try{
    const ctrl=new AbortController(); setTimeout(()=>ctrl.abort(),10_000);
    const r=await fetch('https://places.googleapis.com/v1/places:searchText',{method:'POST',signal:ctrl.signal,
...
async function handleOverpass(req,res) {
  ...
  try{
    ...
    const r=await fetch('https://overpass-api.de/api/interpreter',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:`data=${encodeURIComponent(body.trim())}`,signal:ctrl.signal});
    const buf=Buffer.from(await r.arrayBuffer());
    res.setHeader('Cache-Control','public, max-age=300');
    return sr(res,r.status,buf,r.headers.get('content-type')||'application/json');
  }catch(err){return sj(res,502,{error:err.message});}
}
...
async function handleRoute(req,res) {
  ...
  try{
    const ctrl=new AbortController(); setTimeout(()=>ctrl.abort(),15_000);
    const r=await fetch(`https://router.project-osrm.org/route/v1/${osrmMode}/${encodeURIComponent(coords)}?overview=full&geometries=geojson&steps=false`,{headers:{'User-Agent':'gods-eye-view/1.0'},signal:ctrl.signal});
    res.setHeader('Cache-Control','public, max-age=300');
...
// ─────────────────────────────────────────────────────────────────────────────
// TERRAIN HEIGHTS — Fixed: GET to terrain.reearth.land/heights.json?points=lon,lat;...
// ─────────────────────────────────────────────────────────────────────────────
...
async function handleTerrainHeights(req,res) {
  ...
  // Validate points (lon,lat pairs separated by semicolons — 5dp precision)
  let points;
  try{
    points=rawPoints.split(';').map(s=>{
      const parts=s.split(','); if(parts.length<2)throw new Error();
  ...
  // Per-point cache keyed at 5dp precision (matches client)
  const now=Date.now();
  const canonKey=points.map(([lon,lat])=>`${lon.toFixed(5)},${lat.toFixed(5)}`).join(';');
  const cached=_terrainCache.get(canonKey);
  if(cached&&now-cached.at<30*24*3600_000)return sj(res,200,cached.data);

  try{
    // Correct upstream: GET terrain.reearth.land/heights.json?points=lon,lat;...
    const upUrl=`https://terrain.reearth.land/heights.json?points=${encodeURIComponent(rawPoints)}`;
    const ctrl=new AbortController(); setTimeout(()=>ctrl.abort(),30_000);
    const r=await fetch(upUrl,{signal:ctrl.signal});
    if(!r.ok)throw new Error(`HTTP ${r.status}`);
    ...
    if(cached)return sj(res,200,cached.data);
    return sj(res,502,{error:err.message});
  }
}

// ─────────────────────────────────────────────────────────────────────────────
// MILITARY INSTALLATIONS — Fixed: returns retrievedAt + elements array
// ─────────────────────────────────────────────────────────────────────────────
...
async function handleMilitaryInstallations(req,res) {
  ...
  // Snap box to 0.5-degree grid (matches vite.config quantize behavior for cache sharing)
  ...
  if(cached&&now-cached.at<3600_000){
    res.setHeader('Cache-Control','public, max-age=60');
    return sj(res,200,{...cached.payload,status:'cached'});
  }
  try{
    ...
    const ctrl=new AbortController(); setTimeout(()=>ctrl.abort(),30_000);
    const r=await fetch('https://overpass-api.de/api/interpreter',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:`data=${encodeURIComponent(query)}`,signal:ctrl.signal});
    if(!r.ok)throw new Error(`Overpass HTTP ${r.status}`);
    const data=await r.json();
    // Match the exact payload shape normalizeMilitaryInstallations expects
...
async function handleFirms(req,res) {
  ...
  if(!mapKey)return sj(res,503,{error:'FIRMS_MAP_KEY is not set'});
  const now=Date.now();
  if(_firmsCache&&now-_firmsCache.at<30*60_000){const fires=filterFirms24h(_firmsCache.fires,now);return sj(res,200,{fetchedAt:_firmsCache.at,stale:false,sources:_firmsCache.sources,count:fires.length,fires});}
  const sources=[],fires=[];
  for(const source of FIRMS_SOURCES){
    try{
      const ctrl=new AbortController(); setTimeout(()=>ctrl.abort(),60_000);
      const r=await fetch(`https://firms.modaps.eosdis.nasa.gov/api/area/csv/${encodeURIComponent(mapKey)}/${source}/world/2`,{signal:ctrl.signal});
      if(!r.ok)throw new Error(`HTTP ${r.status}`);
  ...
  if(!sources.some(s=>s.ok)){
    if(_firmsCache){const f=filterFirms24h(_firmsCache.fires,now);return sj(res,200,{fetchedAt:_firmsCache.at,stale:true,sources:_firmsCache.sources,count:f.length,fires:f});}
    return sj(res,502,{error:'all FIRMS sources failed'});
  }
  _firmsCache={at:now,sources,fires};
  return sj(res,200,{fetchedAt:now,stale:false,sources,count:fires.length,fires});
}
...
async function handleRegionalBrief(req,res) {
  ...
  try{
    ...
    setTimeout(()=>ctrl1.abort(),8_000); setTimeout(()=>ctrl2.abort(),8_000);
    const [weather,place]=await Promise.allSettled([
      fetch(wxUrl.toString(),{signal:ctrl1.signal}).then(r=>r.ok?r.json():null),
      fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${latitude}&lon=${longitude}&zoom=10`,{headers:{'User-Agent':'gods-eye-view/1.0'},signal:ctrl2.signal}).then(r=>r.ok?r.json():null),
    ]);
...
async function handleWeatherEffects(req,res) {
  ...
  try{
    ...
    wxUrl.searchParams.set('temperature_unit','celsius');
    const ctrl=new AbortController(); setTimeout(()=>ctrl.abort(),8_000);
    const r=await fetch(wxUrl.toString(),{signal:ctrl.signal}); if(!r.ok)throw new Error(`Open-Meteo HTTP ${r.status}`);
...
// ─────────────────────────────────────────────────────────────────────────────
// TOMTOM — tile TTL extended to 120s to match client decode cache
// ─────────────────────────────────────────────────────────────────────────────
...
async function handleTomtom(req,res) {
  ...
  if(entry&&now-entry.at<120_000)return sr(res,200,entry.buf,'application/x-protobuf'); // 120s TTL
  if(!tomtomOk()){if(entry)return sr(res,200,entry.buf,'application/x-protobuf');return sj(res,429,{error:'budget exhausted'});}
  try{
    const ctrl=new AbortController(); setTimeout(()=>ctrl.abort(),10_000);
    const r=await fetch(`https://api.tomtom.com/traffic/map/4/tile/flow/relative0/${z}/${x}/${y}.pbf?key=${process.env.TOMTOM_API_KEY}&thickness=10`,{signal:ctrl.signal});
    if(!r.ok)return sj(res,502,{error:'TomTom upstream error'});
    const buf=Buffer.from(await r.arrayBuffer());
    _tileCache.set(tileKey,{at:now,buf}); _dailyCount++;
    return sr(res,200,buf,'application/x-protobuf');
  }catch{if(entry)return sr(res,200,entry.buf,'application/x-protobuf');return sj(res,502,{error:'TomTom proxy error'});}
}
...
// ─────────────────────────────────────────────────────────────────────────────
// CCTV — handles /sources /health /stream/:id /frame/:id /media/:id
// ─────────────────────────────────────────────────────────────────────────────
...
async function getCctvSources(){
  ...
  try{
    const ctrl=new AbortController(); setTimeout(()=>ctrl.abort(),15_000);
    const r=await fetch('https://data.austintexas.gov/resource/b4k4-adkb.json?$limit=100&$select=camera_id,location_latitude,location_longitude,camera_status',{signal:ctrl.signal});
    if(!r.ok)throw new Error(`HTTP ${r.status}`);
...
async function handleCctv(req,res) {
  ...
  try{
    ...
    if(sub==='sources'||sub===''){
      res.setHeader('Cache-Control','no-store');
      return sj(res,200,{sources:sources.map(s=>({id:s.id,name:s.name,city:s.city,cityId:s.cityId,provider:s.provider,lat:s.lat,lon:s.lon,headingDeg:s.headingDeg,pitchDeg:s.pitchDeg,fovDeg:s.fovDeg,rangeM:s.rangeM,feedType:s.feedType||'image',sourceKind:s.sourceKind||'fallback'}))});
    }
    ...
    if(sub.startsWith('stream/')){
      ...
      res.setHeader('Cache-Control','no-store');
      return sj(res,200,{id:cameraId,feedType:source?.feedType||'image',mediaUrl:null,frameUrl:`/api/cctv/frame/${encodeURIComponent(cameraId)}`,provider:source?.provider||'',sourceKind:source?.sourceKind||'fallback'});
    }
    if(sub.startsWith('frame/')){
      ...
      if(svKey&&Number.isFinite(lat)&&Number.isFinite(lon_)){
        try{
          ...
          const ctrl=new AbortController(); setTimeout(()=>ctrl.abort(),8_000);
          const svResp=await fetch(sv.toString(),{headers:{'User-Agent':'gods-eye-view-cctv-proxy/1.0'},signal:ctrl.signal});
...
async function handleGbfs(req,res) {
  ...
  try{
    const ctrl=new AbortController(); setTimeout(()=>ctrl.abort(),10_000);
    const r=await fetch(upstreamUrl.toString(),{signal:ctrl.signal});
...
// ─────────────────────────────────────────────────────────────────────────────
// RADIO — Fixed: proper /stations with correct normalization matching the app's
//         strict filter (lastcheckok=1, no HLS, HTTPS only, has geo, valid codec)
// ─────────────────────────────────────────────────────────────────────────────
...
function publicRadioHttpsUrl(value){try{const url=new URL(String(value??''));const h=url.hostname.toLowerCase().replace(/^\[|\]$/g,'').replace(/\.$/,'');if(url.protocol!=='https:'||url.username||url.password||!h)return null;if(h==='localhost'||h.endsWith('.localhost')||h.endsWith('.local'))return null;url.hash='';return url.href;}catch{return null;}}
...
function normalizeStation(raw){
  ...
  return{id,name,lat,lon,streamUrl,homepage:publicRadioHttpsUrl(raw?.homepage),tags,languages,state:cleanRadioText(raw?.state,80),country:cleanRadioText(raw?.country,80),countryCode,metadataTrust:'untrusted-community',codec,bitrate:Number.isInteger(bitrate)&&bitrate>=8&&bitrate<=1024?bitrate:null,clickCount:Math.max(0,Math.min(10_000_000,Number(raw?.clickcount)||0))};
}
...
async function fetchRadioPath(pathname){
  ...
  for(const origin of origins){
    try{
      const ctrl=new AbortController(); setTimeout(()=>ctrl.abort(),15_000);
      const r=await fetch(`${origin}${pathname}`,{headers:{'User-Agent':'gods-eye-view/1.0','Accept':'application/json'},signal:ctrl.signal});
      if(!r.ok)throw new Error(`HTTP ${r.status}`);
      return await r.json();
...
async function buildRadioCatalog(){
  ...
  // Specialist queries first, then popularity fill
  ...
  return{stations:selected.map(s=>({id:s.id,name:s.name,lat:s.lat,lon:s.lon,streamUrl:s.streamUrl,homepage:s.homepage,tags:s.tags,languages:s.languages,state:s.state,country:s.country,countryCode:s.countryCode,metadataTrust:s.metadataTrust,codec:s.codec,bitrate:s.bitrate})),updatedAt:new Date().toISOString(),stale:false,degraded:false,degradedReason:null};
}
...
async function handleRadio(req,res){
  ...
  if(sub==='stations'){
    ...
    try{
      ...
      // Build served-ids set for click validation
      for(const s of catalog.stations)_radioServedIds.add(s.id);
      res.setHeader('Cache-Control','no-store');
      return sj(res,200,catalog);
    }catch(err){
      return sj(res,503,{error:'Radio directory temporarily unavailable',degraded:true,degradedReason:err.message});
    }
  }
  ...
  // Forward other paths directly to radio-browser
  try{
    const fwdPath=url.pathname.replace(/^\/api\/radio/,'')||'/json/stations/topclick';
    const ctrl=new AbortController(); setTimeout(()=>ctrl.abort(),10_000);
    const r=await fetch(`https://all.api.radio-browser.info${fwdPath}${url.search}`,{headers:{'User-Agent':'gods-eye-view/1.0','Accept':'application/json'},signal:ctrl.signal});
    res.setHeader('Cache-Control','public, max-age=300');
